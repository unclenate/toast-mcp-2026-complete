...
| M3 | Monetary input bounds (`z.number().int().positive().max(N)`) on `refundAmount`, cash `amount`, and any other unbounded number | 2026-06-15 | complete (ADR-0003 PR #5 `ec9b306` + impl PR #6 `30315217`, 2026-05-29/30) | Track 3 |
| M4 | HTTP server bound to `127.0.0.1` by default (env opt-in to bind elsewhere); explicit CORS allow-list replacing wildcard | 2026-06-22 | in progress (ADR-0004 accepted 2026-05-31; implementation pending) | Track 3 |
